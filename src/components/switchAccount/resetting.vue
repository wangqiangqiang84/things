		...
		<div class="accountTemplate-center">
			<ul>
				...
				<li>
					...
					<input @blur="userNameOnblurFn()" type="text" placeholder="请输入用户名" v-model="username" onKeypress="javascript:if(event.keyCode == 32)event.returnValue = false;"/>
					<div class="codeOut" v-if="!unbind" @mouseenter="HighlightCode" @mouseleave="cancelHighlightCode" ref="Highlight">
						<span v-show="show" class="count">{{count}}s</span>
						<span class="obtainCode" @click="obtainCodes" v-text="showCodeText"></span>
					</div>
				...
				<li>
					<button type="button" class="resetIcon" @click="resettingBtn">重置</button>
				</li>
			</ul>
			<div class="accountTemplate-center-bottom">
			    <p v-show="showTishi">{{tishi}}</p>
			    <b><button type="button" @click="clickLoginFn">登录</button><button type="button" @click="clickRegisterFn">注册</button></b>
		    </div>
		</div>
	</div>
</template>
...
export default {
  ...
  methods: {
        ...
        if (!this.timer) {
          ...
          this.timer = setInterval(() => {
            if (this.count > 1 && this.count <= TIME_COUNT) {
              ...
              this.$http.post('/api/noauth/resendCode', {email: email}).then(response => {
              // get status
              // get body data
                //  console.log("ok")
              ...
              // error callback
                if(response.status === 401) {
                  this.tishi = response.body.message
                }
              })
            }
          }, 1000)
          ...
          this.$http.post('/api/noauth/getVerificationCode', {email: email}).then(response => {
          // get status
          // get body data
            //  console.log("ok")
            if(response.status === 200) {
    ...
    moveFocus (ev) {
      ...
      let code = this.code
      let email = sessionStorage.getItem('username')
      this.$http.post('/api/noauth/verifyCode', {email: email, code: code}).then(response => {
         console.log(response.body)
        if(response.status === 200) {
    ...
    resettingBtn () {
      ...
      if (!this.username) {
        ...
        this.tishi = '请输入验证码'
      } else if (this.username === '' || this.newPassword === '' || this.code === '') {
        this.tishi = '请完善信息'
      }
      let email = sessionStorage.getItem('username')
      let newPassword = this.newPassword
      this.$http.post('/api/noauth/resetPasswordByEmail', {email: email}).then(response => {
         if(response.status === 200) {
          this.tishi = response.body.message
  ...
  mounted () {
    $(function () {
      ...
      $('#resetCode input').focus(function () {
        $(this).css('backgroundColor', 'rgba(255,255,255,0.5)')
      })
      $('#resetCode input').blur(function () {
...
<style>
  @import "../../assets/css/switchAccount/resetting.css";
</style>
